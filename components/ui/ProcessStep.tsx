...
export function ProcessStep({ step, className }: ProcessStepProps) {
  return (
    <div className={cn(
      "process-node relative flex flex-col items-center text-center p-8 bg-white rounded-3xl z-10 w-full md:max-w-[320px] lg:max-w-[340px] shadow-[0_4px_24px_rgba(106,58,42,0.06)] border border-border group overflow-hidden transition-transform duration-300 hover:-translate-y-1 hover:shadow-lg", 
      className
    )}>
      ...
      {/* Cerchio del Numero */}
      <div className="w-12 h-12 rounded-full bg-teal-light border-2 border-teal flex items-center justify-center mb-6 shadow-sm shrink-0 relative z-10">
      ...
      <div className="flex flex-col items-center gap-3 mb-4 relative z-10">
        ...
        <h3 className="font-heading font-bold text-brown text-[20px] md:text-[22px] leading-tight text-balance">
          {step.titolo}
        </h3>
      </div>
      
      {/* Copy Descrizione */}
      <p className="font-sans text-text-sec text-[15px] font-normal leading=[1.65] relative z-10 text-balance">
        {step.descrizione}
      </p>
